...
bool set_extent_buffer_dirty(struct extent_buffer *eb)
{
	...
	if (!was_dirty) {
		 ...
		 */
		if (subpage)
			lock_page_debug(eb->pages[0]);
		for (i = 0; i < num_pages; i++)
			btrfs_page_set_dirty(eb->fs_info, eb->pages[i],
					     eb->start, eb->len);
		if (subpage)
			unlock_page_debug(eb->pages[0]);
	}
#ifdef CONFIG_BTRFS_DEBUG
	for (i = 0; i < num_pages; i++)
