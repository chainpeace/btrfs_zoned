			       ...
			       struct blk_zone *zones, unsigned int *nr_zones)
{
	...
	*nr_zones = ret;
	...
	unsigned int i;
	for(i=0;i<*nr_zones;i++){
		zones[i].capacity = rounddown_pow_of_two(zones[i].capacity);
		//ihhwang
		print_cap(zones[i].capacity);
	}
			   ...
			   int rw, u64 *bytenr_ret)
{
	...
	if (rw == WRITE) {
		...
		u64 zone_end = 0;
		//ihhwang
		print_cap(zones[0].capacity);
		print_cap(zones[1].capacity);
	...
	for (i = 0; i < BTRFS_NR_SB_LOG_ZONES; i++) {
		...
		if (sb_zone_is_full(zone)) {
			...
			int ret;
				...
				//printk("(%s):%d full condition : wp != start + cap", __func__, __LINE__);
...
bool btrfs_zone_activate(struct btrfs_block_group *block_group)
{
	...
	/* No space left */
//ihhwang	
	print_cap(block_group->zone_capacity);
